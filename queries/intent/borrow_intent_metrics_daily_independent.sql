-- ============================================================
-- Query: borrow_intent_metrics_daily_independent
-- Description: Daily intent metrics and concentration statistics
-- ============================================================
...
SELECT
    ...
    y.signal_offramp_7d,
    CASE
        WHEN y.signal_loop_24h THEN 'A_looper_24h'
        WHEN y.signal_loop_1h THEN 'A_looper_1h'
        WHEN y.signal_bridge_7d THEN 'C_bridge'
        WHEN y.signal_offramp_7d THEN 'D_offramp_proxy'
        WHEN y.signal_defi_7d THEN 'B_defi_retained'
        ELSE 'E_idle_or_unknown'
...
FROM (
    SELECT
        ...
        c.protocol,
        c.borrow_time_utc,
        c.borrow_time_utc AS event_time_utc,
        CAST(date_trunc('day', c.borrow_time_utc) AS date) AS day_utc,
        c.borrow_tx_hash,
        c.evt_index,
        lower(to_hex(c.borrower_address)) AS borrower,
        lower(to_hex(c.receiver_address)) AS receiver,
        lower(to_hex(c.debt_token_address)) AS debt_token_address,
        c.debt_amount_raw / power(10, coalesce(dt.decimals, 18)) AS borrow_amount,
        (c.debt_amount_raw / power(10, coalesce(dt.decimals, 18))) * coalesce(pp.price, 0) AS borrow_amount_usd,
    ...
    LEFT JOIN tokens.erc20 dt
      ON dt.blockchain = c.chain
     AND dt.contract_address = c.debt_token_address
    LEFT JOIN prices.usd pp
      ON pp.blockchain = c.chain
     AND pp.contract_address = c.debt_token_address
     AND pp.minute = date_trunc('minute', c.borrow_time_utc)
...
borrower_daily AS (
    SELECT
        ...
        protocol,
        borrower,
        sum(borrow_amount_usd) AS borrower_borrow_usd,
        count(*) AS borrower_borrow_events
    FROM classified
    GROUP BY 1, 2, 3, 4
),
top25_daily AS (
    SELECT
        ...
        chain,
        protocol,
        sum(borrower_borrow_usd) AS top25_borrow_usd
    FROM (
        SELECT
            b.*,
            row_number() OVER (
                PARTITION BY b.day_utc, b.chain, b.protocol
                ORDER BY b.borrower_borrow_usd DESC
            ) AS rn
        FROM borrower_daily b
    ) x
    WHERE rn <= 25
    GROUP BY 1, 2, 3
),
daily AS (
    SELECT
        ...
        c.protocol,
        count(*) AS n_borrows,
        count(DISTINCT c.borrower) AS n_borrowers,
        sum(c.borrow_amount_usd) AS borrow_usd,
        sum(CASE WHEN c.signal_loop_1h THEN c.borrow_amount_usd ELSE 0 END) / nullif(sum(c.borrow_amount_usd), 0) AS ls1_usd_share,
        sum(CASE WHEN c.signal_loop_24h THEN c.borrow_amount_usd ELSE 0 END) / nullif(sum(c.borrow_amount_usd), 0) AS ls24_usd_share,
        sum(CASE WHEN c.signal_defi_7d THEN c.borrow_amount_usd ELSE 0 END) / nullif(sum(c.borrow_amount_usd), 0) AS defi7_usd_share,
        sum(CASE WHEN c.signal_bridge_7d THEN c.borrow_amount_usd ELSE 0 END) / nullif(sum(c.borrow_amount_usd), 0) AS bridge7_usd_share,
        sum(CASE WHEN c.signal_offramp_7d THEN c.borrow_amount_usd ELSE 0 END) / nullif(sum(c.borrow_amount_usd), 0) AS offramp7_usd_share,
        sum(CASE WHEN c.signal_loop_24h THEN 1 ELSE 0 END) / nullif(cast(count(*) AS double), 0) AS loop24_event_rate,
        sum(CASE WHEN c.intent_bucket = 'A_looper_24h' OR c.intent_bucket = 'A_looper_1h' THEN 1 ELSE 0 END) / nullif(cast(count(*) AS double), 0) AS loop_event_rate
    FROM classified c
    GROUP BY 1, 2, 3
)
SELECT
    ...
    CAST(d.day_utc AS timestamp) AS event_time_utc,
    d.day_utc,
    d.n_borrows,
    d.n_borrowers,
    d.borrow_usd,
    ...
    d.defi7_usd_share,
    d.bridge7_usd_share,
    d.offramp7_usd_share,
    d.loop24_event_rate,
    d.loop_event_rate,
    coalesce(t.top25_borrow_usd, 0) / nullif(d.borrow_usd, 0) AS top25_share,
    sum(CASE WHEN b.borrower_borrow_events >= 3 THEN 1 ELSE 0 END) / nullif(cast(count(*) AS double), 0) AS repeat_rate,
    'ALL' AS intent_bucket,
    'daily_aggregate' AS intent_reason,
    ...
    m.parameter_hash
FROM daily d
LEFT JOIN top25_daily t
  ON t.day_utc = d.day_utc
 AND t.chain = d.chain
 AND t.protocol = d.protocol
LEFT JOIN borrower_daily b
  ON b.day_utc = d.day_utc
 AND b.chain = d.chain
 AND b.protocol = d.protocol
CROSS JOIN meta m
GROUP BY
    d.day_utc, d.chain, d.protocol, d.n_borrows, d.n_borrowers, d.borrow_usd,
    d.ls1_usd_share, d.ls24_usd_share, d.defi7_usd_share, d.bridge7_usd_share,
    d.offramp7_usd_share, d.loop24_event_rate, d.loop_event_rate, t.top25_borrow_usd,
    m.query_version, m.as_of_utc, m.parameter_hash
ORDER BY d.day_utc, d.chain, d.protocol;
